...
type OverlayConfig struct {
	...
	// CreatorUID / CreatorName / CreatorAvatar / CreatedAt surface authorship +
	// creation time for display only (OCT-179/OCT-187). Authorship still resolves
	// via __ODOC_CAP__.isAuthor; never derive it from creator_uid. omitempty keeps
	// legacy __ODOC__ bytes for docs without a stamped creator or a draft with no
	// published version. CreatorName / CreatorAvatar are resolved at render time
	// by looking CreatorUID up through octoidentity (with a small LRU/TTL cache);
	// lookup miss / provider disabled ⇒ both stay empty and the frontend falls
	// back to the raw uid.
